...
class BaseConfig:
    def __init__(
        ...
        self.user_cfg_filename = (
            absolute_path(fn)
            if fn
            else path.join(user_config_dir(app_name), "config.json")
        )
    ...
    def load_env(self):
        ...
        for key, value in (
            (k[n:], v) for k, v in os.environ.items() if k.startswith(self.env_prefix)
        ):
            key = key.upper()
            if hasattr(self, key):
    ...
    def string_import(dotted_path):
        ...
        try:
            ...
            raise ImportError(
                'Module "{}" has no "{}" attribute'.format(mod_path, attr)
            )
...
class PrologConfig(BaseConfig):
    ...
    COLOR_LONG_FMT = (
        "{color}[{asctime} {name}:{levelname}:{module}:{lineno}]{endcolor} {message}"
    )
...
def dict_config(
    loggers=None, level=None, propagate=None, disable_existing=None, cfg=None
):
    """
